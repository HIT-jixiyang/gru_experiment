...
if __name__ == '__main__':
    ...
    metric_path=os.path.join('/extend/gru_tf_data/cross_convgru/Test/79999test/metrics')

    length=10
    evaluator = Evaluator(metric_path,seq=length)
    if not os.path.exists(metric_path):
        os.mkdir(metric_path)
    path='/extend/gru_tf_data/cross_convgru/Test/79999test'
    time=['201904110000','201905312300']
    ...
    for i in range(len(time_range)):
        ...
        if os.path.exists(save_path):
            ...
            for out_png in range(1,length+1):
                out[0,out_png-1,:,:,0]=np.array(Image.open(save_path+'/out/{}.png'.format(out_png)).convert("L"))[100:-100,:]
            for pred_png in range(1,length+1):
                pred[0,pred_png-1,:,:,0]=np.array(Image.open(save_path+'/hist/{}.png'.format(pred_png)).convert("L"))[100:-100,:]
            if len(out[out>0])>2000*1*length:
