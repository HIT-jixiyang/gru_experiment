...
def origin_transfer(img_path, des_path):
...
def array2GRAY_unpadding(img, des_path):
    """Transform the input image array to gray mode and save to a specified destination
    Parameters
    ----------
    img_path : path to a image, this image must be a gray level image.
...
def control_flow1(dir_path, out_path):
    ...
    for dir in os.listdir(dir_path):
        if dir.endswith('colored'):
            continue
        in_p = os.path.join(dir_path, dir)
        out_p = os.path.join(out_path, dir + "_colored")
        multi_process_transfer(in_p, out_p)
    print('All works have been done.')


if __name__ == "__main__":
    from hist_seq.model import Model as hist_model
    # img_path = "output_screenshot_31.08.2018.png"
    # des_path = "."
    # a = time.time()
