...
if __name__ == '__main__':
    ...
    metric_path=os.path.join('/extend/gru_tf_data/gru_experiment/ln_multi_scale_221/Valid/139999valid-metric-hist')

    length=20
    evaluator = Evaluator(metric_path,seq=length)
    if not os.path.exists(metric_path):
        os.mkdir(metric_path)
    path='/extend/gru_tf_data/gru_experiment/ln_multi_scale_221/Valid/139999valid'
    # out_path='/extend/gru_tf_data/3_layer_7_5_3/Test/112000-20200327'
    time=['202003240630','202003241254']
    ...
    for i in range(1,1001):
        ...
        # save_out_path=os.path.join(path,date)
        if os.path.exists(save_path):
            try:
                print(save_path)
                out=np.zeros((1,length,360,360,1))
                pred=np.zeros((1,length,360,360,1))
                for out_png in range(1,length+1):
                    out[0,out_png-1,:,:,0]=np.array(Image.open(save_path+'/out/{}.png'.format(out_png)).convert("L"))
                for pred_png in range(1,length+1):
                    pred[0,pred_png-1,:,:,0]=np.array(Image.open(save_path+'/hist/{}.png'.format(pred_png)).convert("L"))
            except:
                print('---')
                continue
            count+=1
